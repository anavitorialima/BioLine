...
document.addEventListener('DOMContentLoaded', () => {
        ...
        onValue(postsRef,(snapshot)=>{
            ...
            if(posts){
                ...
                postIds.forEach((postId)=>{
                    ...
                    const postElement = document.createElement('div')
                    postElement.innerHTML = `
                        <button class="btn btn-danger btn-sm mx-3 delete-post" data-id="${post.titulo}"><i class="bi bi-trash3-fill"></i>Apagar</button>
                ...
                document.querySelectorAll('.delete-post').forEach((button)=>{
                    button.addEventListener('click',(e)=>{
                        const postId = e.target.getAtribute('data-id')
                        apagarPost(postId)
                    })
                })
            }else{
    ...
    const apagarPost = (postId)=>{
         remove(databaseRef(database,`posts/${postId}`)).then(()=>{
         ...
         .catch((error)=>{
            ...
            alert('Erro ao tentar remove o post')
         })
    }
